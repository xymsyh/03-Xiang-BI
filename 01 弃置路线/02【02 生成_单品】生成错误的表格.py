...
(function(){
    ...
    basename = os.path.splitext(os.path.basename(file_path))[0]
    table_rows = []
    for idx, p in enumerate(product_groups, 1):
        est_60 = int(p["qty"] * ESTIMATED_60DAY_MULTIPLIER)
        turnover = round(p["stock"] / est_60, 2) if est_60 else None
        # 详情页链接（相对路径）
        safe_name = re.sub(r'[<>:"/\\|?*]', '', p["name"])
        sales_fmt = format_number(p["sales"])
        qty_fmt = format_number(p["qty"])
        stock_fmt = format_number(p["stock"])
        price_fmt = f"{p['price']:.2f}".rstrip('0').rstrip('.')
        detail_file = f"{idx:02d}【{sales_fmt}  {qty_fmt}  {stock_fmt}  {price_fmt}】{safe_name}.html"
        detail_href = f"{basename}/{detail_file}"
        table_rows.append({
            "idx": idx, "name": p["name"], "sales": p["sales"],
            "qty": p["qty"], "est60": est_60, "stock": p["stock"],
            "price": p["price"], "turnover": turnover, "href": detail_href,
        })

    # 构建 HTML 表格
    tr_html_list = []
    for r in table_rows:
        turnover_str = f'{r["turnover"]:.2f}' if r["turnover"] is not None else "-"
        tr_html_list.append(
            f'<tr>'
            f'<td>{r["idx"]}</td>'
            f'<td style="text-align:left"><a href="{r["href"]}" target="_blank">{r["name"]}</a></td>'
            f'<td>{r["sales"]:,.2f}</td>'
            f'<td>{r["qty"]:,}</td>'
            f'<td>{r["est60"]:,}</td>'
            f'<td>{r["stock"]:,}</td>'
            f'<td>{r["price"]:.2f}</td>'
            f'<td>{turnover_str}</td>'
            f'</tr>'
        )
    rows_html = "\n".join(tr_html_list)

    summary_html = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>{basename} - 商品汇总表</title>
<style>
body{{font-family:'Microsoft YaHei',sans-serif;margin:20px;background:#f8f9fa;}}
h2{{color:#2c3e50;margin-bottom:16px;}}
table{{border-collapse:collapse;width:100%;background:#fff;box-shadow:0 2px 12px rgba(0,0,0,0.08);border-radius:8px;overflow:hidden;}}
thead{{background:#2c7be5;color:#fff;position:sticky;top:0;}}
th{{padding:12px 14px;cursor:pointer;user-select:none;white-space:nowrap;font-size:13px;}}
th:hover{{background:#1a5fc7;}}
th .arrow{{font-size:10px;margin-left:4px;opacity:0.6;}}
td{{padding:10px 14px;border-bottom:1px solid #eee;text-align:right;font-size:13px;white-space:nowrap;}}
tr:nth-child(even){{background:#f7faff;}}
tr:hover{{background:#e8f0fe;}}
a{{color:#2c7be5;text-decoration:none;}}
a:hover{{text-decoration:underline;}}
.summary-bar{{display:flex;gap:20px;flex-wrap:wrap;margin-bottom:18px;}}
.summary-card{{background:#fff;border-radius:10px;padding:14px 24px;box-shadow:0 2px 10px rgba(44,123,229,0.1);text-align:center;min-width:140px;}}
.summary-card .label{{color:#888;font-size:12px;margin-bottom:4px;}}
.summary-card .value{{font-size:22px;font-weight:bold;}}
</style>
</head><body>
<h2>{basename} — 商品汇总表（共 {len(table_rows)} 个商品）</h2>
<div class="summary-bar">
  <div class="summary-card"><div class="label">总销售额</div><div class="value" style="color:#2c7be5;">¥ {sum(r["sales"] for r in table_rows):,.2f}</div></div>
  <div class="summary-card"><div class="label">总销售量</div><div class="value" style="color:#27ae60;">{sum(r["qty"] for r in table_rows):,}</div></div>
  <div class="summary-card"><div class="label">总库存</div><div class="value" style="color:#e67e22;">{sum(r["stock"] for r in table_rows):,}</div></div>
</div>
<table id="summaryTable">
<thead><tr>
  <th data-type="number">排名 <span class="arrow">▲</span></th>
  <th data-type="string">商品名称 <span class="arrow"></span></th>
  <th data-type="number">总销售额(元) <span class="arrow"></span></th>
  <th data-type="number">总销售量(件) <span class="arrow"></span></th>
  <th data-type="number">预估60天销量 <span class="arrow"></span></th>
  <th data-type="number">总库存(件) <span class="arrow"></span></th>
  <th data-type="number">单价(元) <span class="arrow"></span></th>
  <th data-type="number">周转周期(次) <span class="arrow"></span></th>
</tr></thead>
<tbody>
{rows_html}
</tbody>
...
(function(){{
  ...
  var currentCol=0,asc=true;

  function parseVal(td,type){{
    var txt=td.textContent.replace(/[¥,\\s]/g,'');
    if(type==='number'){{var n=parseFloat(txt);return isNaN(n)?-Infinity:n;}}
    return txt.toLowerCase();
  }}

  function sortTable(colIdx){{
    var type=ths[colIdx].getAttribute('data-type')||'string';
    if(colIdx===currentCol){{asc=!asc;}}else{{currentCol=colIdx;asc=true;}}
    var rows=Array.from(tbody.querySelectorAll('tr'));
    rows.sort(function(a,b){{
      var va=parseVal(a.children[colIdx],type);
      var vb=parseVal(b.children[colIdx],type);
    ...
    ths.forEach(function(th,i){{
      ...
      if(i===colIdx){{arrow.textContent=asc?'▲':'▼';arrow.style.opacity='1';}}
      else{{arrow.textContent='';arrow.style.opacity='0.6';}}
    }});
  }}
  ...
  ths.forEach(function(th,i){{
    ...
    summary_file = os.path.join(output_dir, f"{basename}.html")
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write(summary_html)
    print(f"已生成汇总表：{summary_file}")
